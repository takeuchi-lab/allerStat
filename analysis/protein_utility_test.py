...
#from protein_utility.string_alignment import string_alignment
...
def aa_to_numseq(df_aa, aa_list, pbar=0, output_columns=["label", "sequence"]):
    ...
    for index_aa, row_aa in df_aa.iterrows():  # df各行の
        ...
        df_numseq.at[index_aa, "sequence"] = numseq_row[:len(numseq_row)-1]  # 最後の空白削除
    return df_numseq
...
def numseq_to_aa(df_numseq, aa_list, output_columns=["label", "pattern"]):
    ...
    for index_numseq, pattern in tqdm(enumerate(df_numseq["pattern"])):
        aaseq_row = ""
        pattern = str(pattern)
...
def make_Xrow_for_fastwy(i_row, df_sequence, sr_output_pattern, aa_list, freq=False, similarity=False):
    ...
    # print("(i_row: {})initialize X_row: {}".format(i_row, X_row))
    row = df_sequence.loc[i_row]
    seq = row["sequence"]

    for id_output, pattern in sr_output_pattern.iteritems():
        if similarity == False:
            if pattern in seq:
                if freq == False:
                    X_row[id_output] = 1
                    # print("(i_row: {})pattern: {}, seq: {}".format(i_row, pattern, seq))
                else:
                    X_row[id_output] += 1
        else:
    ...
    if(i_row % 100 == 0):
        ...
        if(i_row % 1000 == 0 and i_row != 0):
            print("")
    # print("(i_row: {})X_row: {}".format(i_row, X_row))
